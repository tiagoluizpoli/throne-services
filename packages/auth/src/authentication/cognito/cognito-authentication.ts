...
export class CognitoAuthentication implements Authentication {
  ...
  async signin(
    ...
    try {
      ...
      if (!this.responseHasExpectedChallenge(userPassAuthFlowResponse)) {
        this.logger.error(
          `CognitoAuthentication.signin :: ChallengeName is not MFA_SETUP or SOFTWARE_TOKEN_MFA: ${JSON.stringify(userPassAuthFlowResponse)}`,
        );
  ...
  async initSignin(
    ...
    try {
      ...
      this.logger.info(
        'CognitoAuthentication.initSignin :: User challenged with MFA_SETUP, associating software token',
      );
